...
import { CheckCircle, AlertTriangle, HelpCircle, FileText, Camera, Cpu, Layers, Copy, Check } from 'lucide-react';
...
const VerdictBadge = ({ verdict }) => {
  ...
  return (
    <div className="relative overflow-hidden neu-panel p-8 text-center">
...
const Gauge = ({ score, label }) => {
  return (
    <div className="neu-panel p-6 flex flex-col items-center justify-center">
...
export const ResultDashboard = ({ result }) => {
  ...
  if (result.mode === 'data_extraction') {
    return (
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-4xl mx-auto mt-12 space-y-6 px-6"
      >
        <div className="neu-panel p-8 text-center">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 text-primary bg-background shadow-neu-flat">
            <FileText size={32} />
          </div>
          <h2 className="text-3xl font-bold text-foreground mb-2">Datos Extraídos</h2>
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Información transcrita automáticamente a partir de la imagen proporcionada.
          </p>
        </div>
          ...
          {Object.keys(result.extractedData || {}).length > 0 ? (
            <>
              <div className="flex justify-end mb-6">
                <button 
                  onClick={() => handleCopy(result.extractedData)}
                  className="flex items-center gap-2 neu-button px-5 py-2 text-sm font-medium text-primary transition-colors"
                >
                  {copied ? <Check size={18} /> : <Copy size={18} />}
                  {copied ? 'Copiado al portapapeles' : 'Copiar Información'}
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {Object.entries(result.extractedData).map(([key, value], idx) => (
                  <div key={idx} className="neu-panel p-6 flex flex-col justify-center break-words">
                    <span className="text-xs text-muted-foreground font-bold block mb-1 uppercase tracking-wider">{key}</span>
                    <span className="text-base font-semibold text-foreground">{typeof value === 'object' ? JSON.stringify(value) : value}</span>
                  </div>
                ))}
              </div>
            </>
             ...
             <div className="neu-panel p-8">
               <p className="text-center text-muted-foreground italic">No se encontraron datos de texto estructurado en esta imagen.</p>
             </div>
          )}
        </div>
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 neu-panel p-6 flex flex-col justify-center">
            ...
            <ul className="space-y-3">
              {visualAnalysis.visualSignals.map((signal, idx) => (
                <motion.li 
                  ...
                  className="flex items-start gap-3 text-sm text-muted-foreground shadow-neu-pressed bg-background p-4 rounded-xl"
                >
          ...
          {visualAnalysis.extractedData && Object.keys(visualAnalysis.extractedData).length > 0 && (
            <div className="neu-panel p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button 
                  onClick={() => handleCopy(visualAnalysis.extractedData)}
                  className="p-2 neu-button text-primary rounded-full hover:bg-background/80 transition-colors"
                  title="Copiar datos"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                {Object.entries(visualAnalysis.extractedData).map(([key, value], idx) => (
                  <div key={idx} className="shadow-neu-pressed bg-background p-4 rounded-xl break-words">
